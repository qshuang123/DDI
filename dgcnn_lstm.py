...
logging.basicConfig(level=logging.INFO, filename='dgcnn_lstm.log')
...
def buildmodel():
    ...
    q = DilatedGatedConv1D(rate=5, drop_gate=0.1)([q, mask])
    ...
    # lstm
    z = Bidirectional(GRU(300, dropout=0.5, recurrent_dropout=0.5, activation='relu'))(q)  # (?,?,600)
...
def trainModel(model, Traininput, traininstanceResult, Testinput,testinstanceResult):
    for i in range(100):
        ...
        if f>0.7 :
            F = str(f)
            ModelFName = './model/base_dgcnn_lstm_Model' + "epoch-" + str(i) + "F-" + F + ".h5"
            MODELsaveFILEM = open( ModelFName, "w")
            model.save_weights(ModelFName)

            print("write predict!!")
            FNAME = 'predictions-task' + "Epoch" + str(i) + '.txt'
            PREDICTIONSFILE = open("./predicts/base_dgcnn_lstm_" + FNAME, "w")
            for p in predicted:
                q = p.tolist()
                j = q.index(max(q))
                if j == 0:
