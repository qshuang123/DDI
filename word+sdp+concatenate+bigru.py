...
def buildmodel():
    ...
    # multituple1=[weight, wordVector]
    # x=Lambda(weightedwordvector, output_shape=weightedwordvector_output_shape)(multituple1)
    x=wordVector
    multituple2 = [x, SDPvector]
    x = Lambda(con, output_shape=con_output_shape)(multituple2)
    # lstm
    z = Bidirectional(GRU(300, dropout=0.5, recurrent_dropout=0.5, activation='relu',return_sequences=True))(x)  # (?,?,600)
    ...
    #矩阵
    # multituple13 = [z,s]
    # a = Lambda(batchdotA, output_shape=batchdotA_output_shape)(multituple13)  # (?,154)
    # a = Lambda(softmaxA, output_shape=softmaxA_output_shape)(a)
    # multituple14 = [a, s]
    # b = Lambda(batchdotB, output_shape=batchdotB_output_shape)(multituple14)
    # multituple15 = [z, b]
    # y = Lambda(batchdotC, output_shape=batchdotC_output_shape)(multituple15)
    #
    # # input attention
    v = Dense(1)(z)  # (?,154,1)
    a = Lambda(changeshapeU, output_shape=changeshapeU_output_shape)(v)  # (?,154)
    attention = Lambda(softmaxattention, output_shape=softmax_output_shape)(a)  # (?,154)
    multituple2 = [attention, z]
    d = Lambda(batchdot, output_shape=batchdot_output_shape)(multituple2)  # (?,600)
    output = Dropout(0.5)(d)
...
def trainModel(model, Traininput,Train_Positionweight_input, traininstanceResult, Testinput, Test_Positionweight_input,testinstanceResult,trainSDPinput, testSDPinput):
    for i in range(100):
        print("迭代次数：", i+1)
        model.fit({'position_input': Train_Positionweight_input, 'main_input':Traininput,'SDP_input':trainSDPinput}, {'main_output': traininstanceResult}, epochs=1, batch_size=batch_size)
        ...
        print("predict!!")
        predicted = model.predict({'position_input': Test_Positionweight_input,'main_input':Testinput,'SDP_input':testSDPinput}, verbose=0)
        print("write predict!!")
...
def predict(Test_Positionweight_input,Testinput, testSDPinput):
    for i in range(1):
        ModelFName = '.\\loadmodel\\inputAttentionModelepoch-46F-0.7134860050890585.h5'
        print("load model !!")
        PREDICTIONSFILEM = open( ModelFName, "rt")
        model.load_weights(ModelFName)
        print("predict!!")
        predicted = model.predict(
            {'position_input': Test_Positionweight_input, 'main_input': Testinput, 'SDP_input': testSDPinput},
            verbose=0)
        print("write predict!!")
        FNAME = 'predictions-task' +  "0.71348" + str(i) + '.txt'
...
# 加载实例
    ...
    # 根据模型预测
    # predict(Test_Positionweight_input, Testinput, testSDPinput)    # # evaltuion
    # # for i in range(1):
